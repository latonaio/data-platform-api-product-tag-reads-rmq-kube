...
import (
	dpfm_api_caller "data-platform-api-product-tag-reads-rmq-kube/DPFM_API_Caller"
...
func main() {
	l := logger.NewLogger()
	conf := config.NewConf()
	mongodb, err := database.NewMongoDB(conf.DB)
	...
	caller := dpfm_api_caller.NewDPFMAPICaller(conf, rmq, mongodb)
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncProductTagReads(accepter, &input, &output, l)
	if len(errs) != 0 {
